...
function getNextFriday(date) {
  return new Date(
    date.getTime() + ((5 + 7 - date.getDay()) % 7 || 7) * 24 * 60 * 60 * 1000
  );
}
...
function getWeekNumberByDate(date) {
  ...
  const allDays = (target - startYear) / (24 * 60 * 60 * 1000);
  const weekNumber = Math.ceil((allDays + 1) / 7);
  return weekNumber;
}
...
function getNextFridayThe13th(date) {
  ...
  while (nextFriday.getDate() !== 13 || nextFriday.getDay() !== 5) {
